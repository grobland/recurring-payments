...
plan: 02
subsystem: database
tags: [drizzle, postgres, migration, financial-accounts, supabase]

# Dependency graph
requires:
  - phase: 35-01
    provides: schema.ts with financialAccounts table, accountTypeEnum, accountId FK on statements
provides:
...
affects: [37-account-crud, 38-source-linking]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "DROP INDEX IF EXISTS / CREATE INDEX IF NOT EXISTS — guards for index drift between local migrations and DB state"
    - "Manual drizzle.__drizzle_migrations seeding — repair strategy when journal desync occurs (empty table, all migrations already applied)"

key-files:
  created:
    - src/lib/db/migrations/meta/0011_snapshot.json
  modified:
    - src/lib/db/migrations/0011_famous_stranger.sql (IF EXISTS / IF NOT EXISTS guards added)
    - src/lib/db/migrations/meta/_journal.json (0011 entry added by db:generate in Plan 01, confirmed applied)
