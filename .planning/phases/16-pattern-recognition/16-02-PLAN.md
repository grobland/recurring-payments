...
must_haves:
  truths:
    ...
    - "Pattern detection is automatically triggered after successful import completion"
  artifacts:
    ...
    - path: "src/app/api/import/confirm/route.ts"
      provides: "Updated import confirm that triggers pattern detection"
      exports: ["POST"]
  ...
  key_links:
    ...
    - from: "src/app/api/import/confirm/route.ts"
      ...
      via: "Internal fetch call after import completes"
      pattern: "fetch.*patterns/detect|triggerPatternDetection"
---

<objective>
Create pattern detection and management API endpoints with TanStack Query hooks, enabling the backend infrastructure for detecting recurring charges, fetching suggestions, and handling user accept/dismiss actions. Integrate automatic detection trigger into import flow.
...
Output: 4 API routes (detect, suggestions, accept, dismiss) + 3 TanStack Query hooks + Zod validation schemas + import flow integration
</objective>
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const row of rawPatterns.rows) {
      ...
      // Guess category - verify the returned ID exists in allCategories
      const suggestedCategoryId = guessCategory(pattern.merchantName, allCategories);
      // Note: guessCategory already verifies the category exists before returning its ID
...
2. Verify category guessing returns valid IDs:
   - Start dev server: `npm run dev`
   - Call detect API: `curl -X POST http://localhost:3000/api/patterns/detect -H "Cookie: <session>" -H "Content-Type: application/json" -d "{}" | jq`
   - Verify response shows `stored`, `skipped`, `filtered` counts
   - Call suggestions API: `curl http://localhost:3000/api/patterns/suggestions -H "Cookie: <session>" | jq '.suggestions[].suggestedCategoryId'`
   - Verify returned category IDs are either null or valid UUIDs that exist in the categories table
  </verify>
  <done>Detection API runs window function query and stores patterns; suggestions API returns filtered patterns with verified category IDs</done>
</task>
...
<task type="auto">
  <name>Task 5: Integrate pattern detection trigger into import flow</name>
  <files>src/app/api/import/confirm/route.ts</files>
  <action>
Update `src/app/api/import/confirm/route.ts` to trigger pattern detection after successful import.
...
At the end of the POST handler, after the successful response is prepared but before returning it, add the pattern detection trigger:
...
Add immediately after it (before the return statement):
```typescript
...
This ensures:
- Pattern detection runs automatically after every successful import
- Detection is non-blocking (fire-and-forget)
- Failures in detection don't affect the import response
- Newly imported data is immediately analyzed for patterns
  </action>
  <verify>
1. Run `npx tsc --noEmit` - no TypeScript errors
2. Test the full flow:
   - Start dev server: `npm run dev`
   - Import a PDF with recurring charges
   - Check server logs for "Background pattern detection" entries
   - Verify patterns appear in suggestions API after import
  </verify>
  <done>Import flow automatically triggers pattern detection after successful completion</done>
</task>
...
5. Import flow triggers detection automatically
</verification>
...
- [x] Pattern detection triggers automatically after import completes
- [x] Category guessing returns verified category IDs
</success_criteria>
